...
namespace Wrapper
{
    ...
    /// Предоставляет методы для запуска КОМПАС, создания документов и построения геометрических элементов.
    /// </summary>
    public class Wrapper
    {
        ...
        {
            var plane = (ksEntity)part.NewEntity((short)Obj3dType.o3d_planeOffset);
            var def = (ksPlaneOffsetDefinition)plane.GetDefinition();

            def.SetPlane(part.GetDefaultEntity((short)Obj3dType.o3d_planeXOY));
            def.offset = offset;
            def.direction = true;
